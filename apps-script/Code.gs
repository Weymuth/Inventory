  ...
  try {
    ...
    if (action === 'move') {
      ...
      if (available < request.quantity) {
        throw new Error(
          'Only ' + available + ' units are currently in ' + request.fromState + '.'
        );
      }
...
function normalizePartFlagRequest_(p) {
  ...
  const enabledText = String(
    p.enabled == null ? '' : p.enabled
  ).trim().toLowerCase();

  if (!/^P-\d{6}$/.test(partId)) {
  ...
  if (
    ...
    throw new Error('Invalid part status flag.');
  }

  let enabled;

  if (['1', 'true', 'yes', 'on'].includes(enabledText)) {
    enabled = true;
  } else if (['0', 'false', 'no', 'off'].includes(enabledText)) {
    enabled = false;
  } else {
...
function setPartFlag_(user, request) {
  ...
  try {
    ...
    if (
      h.ProductStatus === undefined ||
      h.Unavailable === undefined ||
      h.NotInventoried === undefined ||
      h.StudyGuide === undefined
    ) {
    ...
    for (let r = 1; r < values.length; r++) {
      if (
        String(values[r][h.PartID] || '')
          .trim()
          .toUpperCase() === request.partId
      ) {
        rowIndex = r + 1;
        row = values[r];
        break;
    ...
    let retired =
      String(row[h.ProductStatus] || '')
        .trim()
        .toUpperCase() === 'RETIRED';

    let unavailable =
      row[h.Unavailable] === true ||
      String(row[h.Unavailable]).toUpperCase() === 'TRUE';

    let notInventoried =
      row[h.NotInventoried] === true ||
      String(row[h.NotInventoried]).toUpperCase() === 'TRUE';

    let studyGuide =
      row[h.StudyGuide] === true ||
      String(row[h.StudyGuide]).toUpperCase() === 'TRUE';

    if (request.flag === 'RETIRED') {
      retired = request.enabled;

      sheet
        .getRange(rowIndex, h.ProductStatus + 1)
        .setValue(retired ? 'Retired' : 'Current');

    } else if (request.flag === 'UNAVAILABLE') {
      unavailable = request.enabled;

      sheet
        .getRange(rowIndex, h.Unavailable + 1)
      ...
      sheet
        .getRange(rowIndex, h.NotInventoried + 1)
        .setValue(notInventoried);

    } else if (request.flag === 'STUDY_GUIDE') {
      studyGuide = request.enabled;

      sheet
        .getRange(rowIndex, h.StudyGuide + 1)
        .setValue(studyGuide);
    }
...
function normalizePartImageRequest_(p) {
  ...
  const imageFormula = imageUrl.match(
    /^=IMAGE\(\s*["'](.+?)["']\s*\)$/i
  );

  if (imageFormula) {
    imageUrl = String(imageFormula[1] || '').trim();
  }

  if (!/^https?:\/\/\S+$/i.test(imageUrl)) {
    throw new Error(
      'Paste a complete http:// or https:// image link.'
    );
  }
...
function setPartImageUrl_(user, request) {
  ...
  try {
    ...
    if (
      h.PartID === undefined ||
      h.ImageURL === undefined
    ) {
      throw new Error('PARTS image columns are missing.');
    }
    ...
    sheet
      .getRange(rowIndex, h.ImageURL + 1)
      .setValue(request.imageUrl);
...
function normalizeMoveRequest_(p) {
  ...
  if (fromState === toState) {
    throw new Error(
      'Source and destination states must be different.'
    );
  }
...
function verifyNonce_(email, request, nonce) {
  if (!nonce) {
    throw new Error(
      'Confirmation token is missing or expired.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Confirmation token does not match this inventory change.'
    );
  }
}
...
function confirmationPage_(
  ...
  const title =
    request.fromState === 'UNCLASSIFIED'
      ? 'Classify inventory'
      : 'Move inventory';
  ...
  const body = `
    <div class="card" id="card">
      ...
      <dl>
        <dt>Part</dt>
        <dd>${escapeHtml_(request.partId)}</dd>

        <dt>Quantity</dt>
        <dd>${request.quantity}</dd>

        <dt>From</dt>
        <dd>${escapeHtml_(request.fromState)}</dd>

        <dt>To</dt>
        <dd>${escapeHtml_(request.toState)}</dd>

        <dt>Current source quantity</dt>
        <dd>${available}</dd>
      </dl>

      <p class="note">
        This first migration step changes inventory state only.
        Physical room/cabinet/bin assignment will be added next.
      </p>

      <button
        id="confirmButton"
        type="button"
        onclick="confirmInventoryChange()"
      >
        Confirm change
      </button>
        ...
        google.script.run
          .withSuccessHandler(function(result) {
            ...
            if (!result || !result.ok) {
              showFailure({
                message:
                  'The inventory change did not return a valid result.'
              });
...
function autoReturnPage_(title, returnUrl) {
  const targetJson =
    JSON.stringify(String(returnUrl))
      .replace(/</g, '\\u003c');
  ...
  return Object.keys(balances)
    ...
    .map(partId => {
      ...
      return [
        ...
        Number(b.storage || 0),
        Number(b.checkedOut || 0),
        Number(b.unclassified || 0)
      ].join(',');
    })
    .join('|');
}
      ...
      <head>
        ...
        <style>
          ...
          .card {
            ...
            margin: auto;
            background: #fff;
            border-top: 6px solid #0B1A2E;
            border-radius: 10px;
            padding: 24px;
            box-shadow:
              0 10px 30px rgba(11,26,46,.12);
          }
