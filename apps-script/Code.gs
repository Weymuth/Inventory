...
function doGet(e) {
  try {
    ...
    if (action === 'partname') {
      const user = requireTeacherOrAdmin_();
      const request = normalizePartNameRequest_(e.parameter);
      const result = setPartName_(user, request);
      const returnUrl = buildFrontendUrl_({
        bridge: 'part-name-updated',
        ok: '1',
        partId: result.partId,
        partName: result.partName
      });
      return autoReturnPage_('Part renamed', returnUrl);
    }
    ...
    if (action === 'partname') {
      const returnUrl = buildFrontendUrl_({
        bridge: 'part-name-error',
        ok: '0',
        partId: String((e && e.parameter && e.parameter.partId) || ''),
        message: safeError_(err)
      });
      return autoReturnPage_('Part rename error', returnUrl);
    }
...
function setPartName_(user, request) {
  ...
  try {
    ...
    if (!values.length) throw new Error('PARTS sheet is empty.');
    const h = headerMap_(values[0]);
    if (h.PartID === undefined || h.PartName === undefined) throw new Error('PARTS name columns are missing.');
    ...
    sheet.getRange(rowIndex, h.PartName + 1).setValue(request.partName);
    SpreadsheetApp.flush();
