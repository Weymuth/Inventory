(function(){
  ...
  function renderRowChoice(row,k,item){
    ...
    if(qty){
      qty.disabled=!d||d.decision!=='APPROVE'||submitting;
      if(d&&d.decision==='APPROVE')qty.value=String(d.approvedQty);
    }
  ...
  function augment(){
    ...
    document.querySelectorAll('.rq-card').forEach(card=>{
      ...
      rows.forEach((row,index)=>{
        ...
        const td=document.createElement('td');td.className='rq-decision-cell';
        ...
        td.querySelector('.rq-approve-qty').oninput=e=>{
          const d=staged[k];if(!d||d.decision!=='APPROVE')return;
          const qty=Number(e.target.value);if(Number.isInteger(qty)&&qty>=1&&qty<=requested)d.approvedQty=qty;
          updateFooter();
        };
        renderRowChoice(row,k,item);
      });
  ...
  function ensureFooter(){
    const body=document.getElementById('requestQueueBody');if(!body||document.getElementById('rqSubmitFooter'))return;
    if(!latestRequests.some(r=>String(r.status||'').toUpperCase()==='SUBMITTED'))return;
    ...
    document.getElementById('rqSubmitDecisions').onclick=submitDecisions;
  }
  ...
  function validStaged(){
    ...
    for(const d of values){
      if(d.decision==='APPROVE'){
        const request=requestById(d.requestId);const item=request&&(request.items||[]).find(i=>i.partId===d.partId);
        const max=item?Number(item.requestedQty||0):0;
  ...
  function submitDecisions(){
    ...
    submitting=true;updateFooter();
    document.querySelectorAll('.rq-item-choice,.rq-approve-qty').forEach(el=>el.disabled=true);
    showMessage('Submitting '+decisions.length+' decision'+(decisions.length===1?'':'s')+'…',false);
    ...
    setTimeout(()=>{try{frame.remove();}catch(e){}},20000);
  }
  ...
  window.addEventListener('message',e=>{
    ...
    if(submitting){
      ...
      setTimeout(()=>showMessage(d.message||'Decisions saved.',false),250);
      return;
    }
